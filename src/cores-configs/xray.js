...
async function buildXrayBestPingConfig(proxySettings, totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? '🔰⚔️ Best Ping ' : '🔰⚔️ - Best Ping ';
...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    const config = buildXrayConfig(proxySettings, '🔰⚔️ Best Fragment 😎', true, true, chainProxy, false, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    const config = buildXrayConfig(proxySettings, '🔰⚔️ WorkerLess ⭐', true, false, false, false, false);
...
export async function getXrayWarpConfigs (request, env, client) {
    ...
    for (const [index, endpoint] of warpEndpoints.split(',').entries()) {
        ...
        const WoWConfig = buildXrayConfig(proxySettings, `🔰⚔️ ${index + 1} - WoW${proIndicator}🌍`, false, false, true, false, true);
    ...
    const xrayWarpBestPing = buildXrayConfig(proxySettings, `🔰⚔️ Warp${proIndicator}- Best Ping 🚀`, false, true, false, false, true);
    ...
    const xrayWoWBestPing = buildXrayConfig(proxySettings, `🔰⚔️ WoW${proIndicator}- Best Ping 🚀`, false, true, true, false, true);
